...
main(void)
{
  ...
  6d:	6a 01                	push   $0x1
  6f:	e8 62 03 00 00       	call   3d6 <printf>
      exit();
  ...
  81:	6a 01                	push   $0x1
  83:	e8 4e 03 00 00       	call   3d6 <printf>
  ...
  a0:	6a 01                	push   $0x1
  a2:	e8 2f 03 00 00       	call   3d6 <printf>
    pid = fork();
  ...
  bc:	68 84 09 00 00       	push   $0x984
  c1:	68 be 06 00 00       	push   $0x6be
  c6:	e8 d9 01 00 00       	call   2a4 <exec>
      printf(1, "init: exec sh failed\n");
  cb:	83 c4 08             	add    $0x8,%esp
  ce:	68 c1 06 00 00       	push   $0x6c1
  d3:	6a 01                	push   $0x1
  d5:	e8 fc 02 00 00       	call   3d6 <printf>
      exit();
...
putc(int fd, char c)
{
 ...
 34a:	52                   	push   %edx
 34b:	50                   	push   %eax
 34c:	e8 3b ff ff ff       	call   28c <write>
}
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 365:	0f 95 c3             	setne  %bl
 368:	89 d0                	mov    %edx,%eax
 36a:	c1 e8 1f             	shr    $0x1f,%eax
 36d:	84 c3                	test   %al,%bl
 36f:	74 10                	je     381 <printint+0x2b>
    neg = 1;
    x = -xx;
 371:	f7 da                	neg    %edx
    neg = 1;
 ...
 37f:	eb 0b                	jmp    38c <printint+0x36>
  neg = 0;
 381:	c7 45 d4 00 00 00 00 	movl   $0x0,-0x2c(%ebp)
 388:	eb f0                	jmp    37a <printint+0x24>
  do{
    buf[i++] = digits[x % base];
 ...
 397:	8d 46 01             	lea    0x1(%esi),%eax
 39a:	0f b6 92 e8 06 00 00 	movzbl 0x6e8(%edx),%edx
 3a1:	88 54 35 d8          	mov    %dl,-0x28(%ebp,%esi,1)
  }while((x /= base) != 0);
 3a5:	89 da                	mov    %ebx,%edx
 3a7:	85 db                	test   %ebx,%ebx
 3a9:	75 df                	jne    38a <printint+0x34>
 3ab:	89 c3                	mov    %eax,%ebx
  if(neg)
 3ad:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
 3b1:	74 16                	je     3c9 <printint+0x73>
    buf[i++] = '-';
 3b3:	c6 44 05 d8 2d       	movb   $0x2d,-0x28(%ebp,%eax,1)
 3b8:	8d 5e 02             	lea    0x2(%esi),%ebx
 3bb:	eb 0c                	jmp    3c9 <printint+0x73>

  while(--i >= 0)
    putc(fd, buf[i]);
 3bd:	0f be 54 1d d8       	movsbl -0x28(%ebp,%ebx,1),%edx
 3c2:	89 f8                	mov    %edi,%eax
 3c4:	e8 73 ff ff ff       	call   33c <putc>
  while(--i >= 0)
 3c9:	83 eb 01             	sub    $0x1,%ebx
 3cc:	79 ef                	jns    3bd <printint+0x67>
}
...
printf(int fd, const char *fmt, ...)
{
 ...
 3f6:	e8 41 ff ff ff       	call   33c <putc>
 ...
 40e:	0f 84 23 01 00 00    	je     537 <printf+0x161>
    c = fmt[i] & 0xff;
 ...
 42a:	74 49                	je     475 <printf+0x9f>
        printint(fd, *ap, 10, 1);
        ap++;
      } else if(c == 'x' || c == 'p'){
 42c:	83 f8 78             	cmp    $0x78,%eax
 42f:	0f 94 c1             	sete   %cl
 432:	83 f8 70             	cmp    $0x70,%eax
 435:	0f 94 c2             	sete   %dl
 438:	08 d1                	or     %dl,%cl
 43a:	75 63                	jne    49f <printf+0xc9>
        printint(fd, *ap, 16, 0);
        ap++;
      } else if(c == 's'){
 43c:	83 f8 73             	cmp    $0x73,%eax
 43f:	0f 84 84 00 00 00    	je     4c9 <printf+0xf3>
          s = "(null)";
        while(*s != 0){
          putc(fd, *s);
          s++;
 ...
 448:	0f 84 b7 00 00 00    	je     505 <printf+0x12f>
        putc(fd, *ap);
        ap++;
      } else if(c == '%'){
 44e:	83 f8 25             	cmp    $0x25,%eax
 451:	0f 84 cc 00 00 00    	je     523 <printf+0x14d>
        putc(fd, c);
      } else {
 ...
 45f:	e8 d8 fe ff ff       	call   33c <putc>
        putc(fd, c);
 464:	89 fa                	mov    %edi,%edx
 466:	8b 45 08             	mov    0x8(%ebp),%eax
 469:	e8 ce fe ff ff       	call   33c <putc>
      }
      state = 0;
 46e:	be 00 00 00 00       	mov    $0x0,%esi
 473:	eb 8d                	jmp    402 <printf+0x2c>
        printint(fd, *ap, 10, 1);
 ...
 487:	e8 ca fe ff ff       	call   356 <printint>
        ap++;
 ...
 49a:	e9 63 ff ff ff       	jmp    402 <printf+0x2c>
        printint(fd, *ap, 16, 0);
 ...
 4b1:	e8 a0 fe ff ff       	call   356 <printint>
        ap++;
 ...
 4c4:	e9 39 ff ff ff       	jmp    402 <printf+0x2c>
        s = (char*)*ap;
 ...
 4d6:	75 28                	jne    500 <printf+0x12a>
          s = "(null)";
 4d8:	be e0 06 00 00       	mov    $0x6e0,%esi
 4dd:	8b 7d 08             	mov    0x8(%ebp),%edi
 4e0:	eb 0d                	jmp    4ef <printf+0x119>
          putc(fd, *s);
 4e2:	0f be d2             	movsbl %dl,%edx
 4e5:	89 f8                	mov    %edi,%eax
 4e7:	e8 50 fe ff ff       	call   33c <putc>
          s++;
 4ec:	83 c6 01             	add    $0x1,%esi
        while(*s != 0){
 4ef:	0f b6 16             	movzbl (%esi),%edx
 4f2:	84 d2                	test   %dl,%dl
 4f4:	75 ec                	jne    4e2 <printf+0x10c>
      state = 0;
 4f6:	be 00 00 00 00       	mov    $0x0,%esi
 4fb:	e9 02 ff ff ff       	jmp    402 <printf+0x2c>
 500:	8b 7d 08             	mov    0x8(%ebp),%edi
 503:	eb ea                	jmp    4ef <printf+0x119>
        putc(fd, *ap);
 505:	8b 7d e4             	mov    -0x1c(%ebp),%edi
 508:	0f be 17             	movsbl (%edi),%edx
 50b:	8b 45 08             	mov    0x8(%ebp),%eax
 50e:	e8 29 fe ff ff       	call   33c <putc>
        ap++;
 513:	83 c7 04             	add    $0x4,%edi
 516:	89 7d e4             	mov    %edi,-0x1c(%ebp)
      state = 0;
 519:	be 00 00 00 00       	mov    $0x0,%esi
 51e:	e9 df fe ff ff       	jmp    402 <printf+0x2c>
        putc(fd, c);
 523:	89 fa                	mov    %edi,%edx
 525:	8b 45 08             	mov    0x8(%ebp),%eax
 528:	e8 0f fe ff ff       	call   33c <putc>
      state = 0;
 52d:	be 00 00 00 00       	mov    $0x0,%esi
 532:	e9 cb fe ff ff       	jmp    402 <printf+0x2c>
    }
  }
}
...
free(void *ap)
{
 ...
 54b:	a1 8c 09 00 00       	mov    0x98c,%eax
 ...
 587:	89 08                	mov    %ecx,(%eax)
  freep = p;
 589:	a3 8c 09 00 00       	mov    %eax,0x98c
}
 58e:	5b                   	pop    %ebx
 58f:	5e                   	pop    %esi
 590:	5f                   	pop    %edi
...
morecore(uint nu)
{
 ...
 5ce:	50                   	push   %eax
 5cf:	e8 20 fd ff ff       	call   2f4 <sbrk>
  if(p == (char*)-1)
 ...
 5e6:	e8 54 ff ff ff       	call   53f <free>
  return freep;
 5eb:	a1 8c 09 00 00       	mov    0x98c,%eax
...
malloc(uint nbytes)
{
 ...
 60f:	83 c3 01             	add    $0x1,%ebx
  if((prevp = freep) == 0){
 612:	8b 0d 8c 09 00 00    	mov    0x98c,%ecx
 ...
 61e:	eb 4d                	jmp    66d <malloc+0x6e>
    base.s.ptr = freep = prevp = &base;
 620:	c7 05 8c 09 00 00 90 	movl   $0x990,0x98c
 627:	09 00 00 
 62a:	c7 05 90 09 00 00 90 	movl   $0x990,0x990
 631:	09 00 00 
    base.s.size = 0;
 634:	c7 05 94 09 00 00 00 	movl   $0x0,0x994
 63b:	00 00 00 
    base.s.ptr = freep = prevp = &base;
 63e:	b9 90 09 00 00       	mov    $0x990,%ecx
 643:	eb d7                	jmp    61c <malloc+0x1d>
    if(p->s.size >= nunits){
      if(p->s.size == nunits)
 645:	39 da                	cmp    %ebx,%edx
 647:	74 1a                	je     663 <malloc+0x64>
        prevp->s.ptr = p->s.ptr;
      else {
        p->s.size -= nunits;
 ...
 651:	89 58 04             	mov    %ebx,0x4(%eax)
      ...
      freep = prevp;
 654:	89 0d 8c 09 00 00    	mov    %ecx,0x98c
      return (void*)(p + 1);
 ...
 65d:	83 c4 04             	add    $0x4,%esp
 660:	5b                   	pop    %ebx
 661:	5d                   	pop    %ebp
 662:	c3                   	ret    
        prevp->s.ptr = p->s.ptr;
 663:	8b 10                	mov    (%eax),%edx
 665:	89 11                	mov    %edx,(%ecx)
 667:	eb eb                	jmp    654 <malloc+0x55>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 669:	89 c1                	mov    %eax,%ecx
 66b:	8b 00                	mov    (%eax),%eax
    if(p->s.size >= nunits){
 66d:	8b 50 04             	mov    0x4(%eax),%edx
 670:	39 da                	cmp    %ebx,%edx
 672:	73 d1                	jae    645 <malloc+0x46>
    if(p == freep)
 674:	39 05 8c 09 00 00    	cmp    %eax,0x98c
 67a:	75 ed                	jne    669 <malloc+0x6a>
      if((p = morecore(nunits)) == 0)
 67c:	89 d8                	mov    %ebx,%eax
 67e:	e8 2c ff ff ff       	call   5af <morecore>
 683:	85 c0                	test   %eax,%eax
 685:	75 e2                	jne    669 <malloc+0x6a>
        return 0;
 687:	b8 00 00 00 00       	mov    $0x0,%eax
 68c:	eb cf                	jmp    65d <malloc+0x5e>
