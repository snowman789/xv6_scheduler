...
main(int argc, char *argv[])
{
  ...
  1a:	68 0c 06 00 00       	push   $0x60c
  1f:	6a 02                	push   $0x2
  21:	e8 2d 03 00 00       	call   353 <printf>
    exit();
  ...
  4b:	68 1f 06 00 00       	push   $0x61f
  50:	6a 02                	push   $0x2
  52:	e8 fc 02 00 00       	call   353 <printf>
...
000002a9 <settickets>:
SYSCALL(settickets)
 2a9:	b8 1a 00 00 00       	mov    $0x1a,%eax
 2ae:	cd 40                	int    $0x40
 2b0:	c3                   	ret    

000002b1 <getprocessesinfo>:
 2b1:	b8 1b 00 00 00       	mov    $0x1b,%eax
 2b6:	cd 40                	int    $0x40
 2b8:	c3                   	ret    
...
putc(int fd, char c)
{
 ...
 2c7:	52                   	push   %edx
 2c8:	50                   	push   %eax
 2c9:	e8 3b ff ff ff       	call   209 <write>
}
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 2dc:	89 c7                	mov    %eax,%edi
  ...
  neg = 0;
  if(sgn && xx < 0){
 2de:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
 2e2:	0f 95 c3             	setne  %bl
 2e5:	89 d0                	mov    %edx,%eax
 2e7:	c1 e8 1f             	shr    $0x1f,%eax
 2ea:	84 c3                	test   %al,%bl
 2ec:	74 10                	je     2fe <printint+0x2b>
    neg = 1;
    x = -xx;
 2ee:	f7 da                	neg    %edx
    neg = 1;
 2f0:	c7 45 d4 01 00 00 00 	movl   $0x1,-0x2c(%ebp)
  } else {
    x = xx;
  }
 ...
 2fc:	eb 0b                	jmp    309 <printint+0x36>
  neg = 0;
 2fe:	c7 45 d4 00 00 00 00 	movl   $0x0,-0x2c(%ebp)
 305:	eb f0                	jmp    2f7 <printint+0x24>
  do{
    buf[i++] = digits[x % base];
 ...
 314:	8d 46 01             	lea    0x1(%esi),%eax
 317:	0f b6 92 3c 06 00 00 	movzbl 0x63c(%edx),%edx
 31e:	88 54 35 d8          	mov    %dl,-0x28(%ebp,%esi,1)
  }while((x /= base) != 0);
 322:	89 da                	mov    %ebx,%edx
 324:	85 db                	test   %ebx,%ebx
 326:	75 df                	jne    307 <printint+0x34>
 328:	89 c3                	mov    %eax,%ebx
  if(neg)
 32a:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
 32e:	74 16                	je     346 <printint+0x73>
    buf[i++] = '-';
 330:	c6 44 05 d8 2d       	movb   $0x2d,-0x28(%ebp,%eax,1)
 335:	8d 5e 02             	lea    0x2(%esi),%ebx
 338:	eb 0c                	jmp    346 <printint+0x73>

  while(--i >= 0)
    putc(fd, buf[i]);
 33a:	0f be 54 1d d8       	movsbl -0x28(%ebp,%ebx,1),%edx
 33f:	89 f8                	mov    %edi,%eax
 341:	e8 73 ff ff ff       	call   2b9 <putc>
  while(--i >= 0)
 346:	83 eb 01             	sub    $0x1,%ebx
 349:	79 ef                	jns    33a <printint+0x67>
}
...
printf(int fd, const char *fmt, ...)
{
 ...
 373:	e8 41 ff ff ff       	call   2b9 <putc>
 ...
 38b:	0f 84 23 01 00 00    	je     4b4 <printf+0x161>
    c = fmt[i] & 0xff;
 ...
 3a7:	74 49                	je     3f2 <printf+0x9f>
        printint(fd, *ap, 10, 1);
        ap++;
      } else if(c == 'x' || c == 'p'){
 3a9:	83 f8 78             	cmp    $0x78,%eax
 3ac:	0f 94 c1             	sete   %cl
 3af:	83 f8 70             	cmp    $0x70,%eax
 3b2:	0f 94 c2             	sete   %dl
 3b5:	08 d1                	or     %dl,%cl
 3b7:	75 63                	jne    41c <printf+0xc9>
        printint(fd, *ap, 16, 0);
        ap++;
      } else if(c == 's'){
 3b9:	83 f8 73             	cmp    $0x73,%eax
 3bc:	0f 84 84 00 00 00    	je     446 <printf+0xf3>
          s = "(null)";
        while(*s != 0){
          putc(fd, *s);
          s++;
 ...
 3c5:	0f 84 b7 00 00 00    	je     482 <printf+0x12f>
        putc(fd, *ap);
        ap++;
      } else if(c == '%'){
 3cb:	83 f8 25             	cmp    $0x25,%eax
 3ce:	0f 84 cc 00 00 00    	je     4a0 <printf+0x14d>
        putc(fd, c);
      } else {
 ...
 3dc:	e8 d8 fe ff ff       	call   2b9 <putc>
        putc(fd, c);
 3e1:	89 fa                	mov    %edi,%edx
 3e3:	8b 45 08             	mov    0x8(%ebp),%eax
 3e6:	e8 ce fe ff ff       	call   2b9 <putc>
      }
      state = 0;
 3eb:	be 00 00 00 00       	mov    $0x0,%esi
 3f0:	eb 8d                	jmp    37f <printf+0x2c>
        printint(fd, *ap, 10, 1);
 ...
 404:	e8 ca fe ff ff       	call   2d3 <printint>
        ap++;
 ...
 417:	e9 63 ff ff ff       	jmp    37f <printf+0x2c>
        printint(fd, *ap, 16, 0);
 ...
 42e:	e8 a0 fe ff ff       	call   2d3 <printint>
        ap++;
 ...
 441:	e9 39 ff ff ff       	jmp    37f <printf+0x2c>
        s = (char*)*ap;
 ...
 453:	75 28                	jne    47d <printf+0x12a>
          s = "(null)";
 455:	be 33 06 00 00       	mov    $0x633,%esi
 45a:	8b 7d 08             	mov    0x8(%ebp),%edi
 45d:	eb 0d                	jmp    46c <printf+0x119>
          putc(fd, *s);
 45f:	0f be d2             	movsbl %dl,%edx
 462:	89 f8                	mov    %edi,%eax
 464:	e8 50 fe ff ff       	call   2b9 <putc>
          s++;
 469:	83 c6 01             	add    $0x1,%esi
        while(*s != 0){
 46c:	0f b6 16             	movzbl (%esi),%edx
 46f:	84 d2                	test   %dl,%dl
 471:	75 ec                	jne    45f <printf+0x10c>
      state = 0;
 473:	be 00 00 00 00       	mov    $0x0,%esi
 478:	e9 02 ff ff ff       	jmp    37f <printf+0x2c>
 47d:	8b 7d 08             	mov    0x8(%ebp),%edi
 480:	eb ea                	jmp    46c <printf+0x119>
        putc(fd, *ap);
 482:	8b 7d e4             	mov    -0x1c(%ebp),%edi
 485:	0f be 17             	movsbl (%edi),%edx
 488:	8b 45 08             	mov    0x8(%ebp),%eax
 48b:	e8 29 fe ff ff       	call   2b9 <putc>
        ap++;
 490:	83 c7 04             	add    $0x4,%edi
 493:	89 7d e4             	mov    %edi,-0x1c(%ebp)
      state = 0;
 496:	be 00 00 00 00       	mov    $0x0,%esi
 49b:	e9 df fe ff ff       	jmp    37f <printf+0x2c>
        putc(fd, c);
 4a0:	89 fa                	mov    %edi,%edx
 4a2:	8b 45 08             	mov    0x8(%ebp),%eax
 4a5:	e8 0f fe ff ff       	call   2b9 <putc>
      state = 0;
 4aa:	be 00 00 00 00       	mov    $0x0,%esi
 4af:	e9 cb fe ff ff       	jmp    37f <printf+0x2c>
    }
  }
}
...
free(void *ap)
{
 ...
 4c8:	a1 d8 08 00 00       	mov    0x8d8,%eax
 ...
 504:	89 08                	mov    %ecx,(%eax)
  freep = p;
 506:	a3 d8 08 00 00       	mov    %eax,0x8d8
}
 50b:	5b                   	pop    %ebx
 50c:	5e                   	pop    %esi
 50d:	5f                   	pop    %edi
...
morecore(uint nu)
{
 ...
 54b:	50                   	push   %eax
 54c:	e8 20 fd ff ff       	call   271 <sbrk>
  if(p == (char*)-1)
 ...
 563:	e8 54 ff ff ff       	call   4bc <free>
  return freep;
 568:	a1 d8 08 00 00       	mov    0x8d8,%eax
...
malloc(uint nbytes)
{
 ...
 58c:	83 c3 01             	add    $0x1,%ebx
  if((prevp = freep) == 0){
 58f:	8b 0d d8 08 00 00    	mov    0x8d8,%ecx
 ...
 59b:	eb 4d                	jmp    5ea <malloc+0x6e>
    base.s.ptr = freep = prevp = &base;
 59d:	c7 05 d8 08 00 00 dc 	movl   $0x8dc,0x8d8
 5a4:	08 00 00 
 5a7:	c7 05 dc 08 00 00 dc 	movl   $0x8dc,0x8dc
 5ae:	08 00 00 
    base.s.size = 0;
 5b1:	c7 05 e0 08 00 00 00 	movl   $0x0,0x8e0
 5b8:	00 00 00 
    base.s.ptr = freep = prevp = &base;
 5bb:	b9 dc 08 00 00       	mov    $0x8dc,%ecx
 5c0:	eb d7                	jmp    599 <malloc+0x1d>
    if(p->s.size >= nunits){
      if(p->s.size == nunits)
 5c2:	39 da                	cmp    %ebx,%edx
 5c4:	74 1a                	je     5e0 <malloc+0x64>
        prevp->s.ptr = p->s.ptr;
      else {
        p->s.size -= nunits;
 ...
 5ce:	89 58 04             	mov    %ebx,0x4(%eax)
      ...
      freep = prevp;
 5d1:	89 0d d8 08 00 00    	mov    %ecx,0x8d8
      return (void*)(p + 1);
 ...
 5ef:	73 d1                	jae    5c2 <malloc+0x46>
    if(p == freep)
 5f1:	39 05 d8 08 00 00    	cmp    %eax,0x8d8
 5f7:	75 ed                	jne    5e6 <malloc+0x6a>
      if((p = morecore(nunits)) == 0)
 5f9:	89 d8                	mov    %ebx,%eax
 5fb:	e8 2c ff ff ff       	call   52c <morecore>
 600:	85 c0                	test   %eax,%eax
 602:	75 e2                	jne    5e6 <malloc+0x6a>
        return 0;
 604:	b8 00 00 00 00       	mov    $0x0,%eax
 609:	eb cf                	jmp    5da <malloc+0x5e>
